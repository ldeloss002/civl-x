use std::{convert::TryInto, ops::Add, str::FromStr, time::Duration};

use log::info;
use num::{bigint::BigInt, rational::BigRational};
use z3::{
    ast::{Array, Ast, Bool, Int, BV},
    *,
};
...
fn get_some_solver_assertions(s: &Solver) {
    let x = ast::Int::new_const(s.get_context(), "x");
    let y = ast::Int::new_const(s.get_context(), "y");
    s.assert(&x.gt(&y));
}
...
fn test_solver_get_assertions_lifetime() {
    ...
    get_some_solver_assertions(&s);
    assert_eq!(s.get_assertions().len(), 1);
}
...
fn test_mutually_recursive_datatype() {
    ...
    let tree_builder = DatatypeBuilder::new(&ctx, "Tree")
        ...
        .variant("node", vec![(
            "children",
            DatatypeAccessor::Datatype("TreeList".into()),
        )]);
...
fn test_goal_apply_tactic() {
    ...
    let goal = Goal::new(&ctx, false, false, false);
    goal.assert(&a_and_b_and_a);
    test_apply_tactic(&ctx, goal, vec![a.clone(), b.clone(), a.clone()], vec![
        a.clone(),
        b.clone(),
    ]);
    ...
    test_apply_tactic(&ctx, goal, vec![a.clone(), a_implies_b.clone()], vec![
        a.clone(),
        b.clone(),
    ]);
    ...
    test_apply_tactic(&ctx, goal, vec![false_bool.clone()], vec![
        false_bool.clone(),
    ]);
}

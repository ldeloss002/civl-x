...
fn path<'tokens, 'src: 'tokens>() -> impl TokenParser<'tokens, 'src, &'src str> {
    select! { Token::Path(path) => path }
        .map_with(|path: &str, e| Spanned::new(path, e.span()))
        .labelled("path")
}
...
pub fn module_parser<'tokens, 'src: 'tokens>() -> impl Parser<
    ...
    extra::Err<Rich<'tokens, Token<'src>, Span>>,
> + Clone {
    let exprs = expr_parser().repeated().collect::<Vec<Spanned<Expr>>>();
